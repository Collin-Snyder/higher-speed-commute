...
const { abs, cos, sin, max, floor, ceil, sqrt, random, PI } = Math;
...
export function calculateSpeedConstant(entity: Entity): number {
  ...
  if (entity.has("SchoolZone")) {
    speed *= entity.SchoolZone.multiplier;
  }
  else if (entity.has("CaffeineBoost")) {
...
export function findCenteredElementSpread(
  ...
  output.start = floor(output.start);
  output.step = floor(output.step);
...
//must be bound to an entity with a hitbox
export function getCurrentHb (degrees?: number) {
  //@ts-ignore
  let entity = <Entity>(<unknown>this);
  if (!entity || !entity.Collision || !entity.Coordinates || !entity.Renderable) {
    console.error(new Error("Trying to run 'getCurrentHb' on an invalid entity"))
    return;
  }
...
//must be bound to an entity with coordinates
export function getCurrentCp() {
  //@ts-ignore
  let entity = <Entity>(<unknown>this);
  if (!entity || !entity.Collision || !entity.Coordinates) {
    console.error(new Error("Trying to run 'getCurrentCp' on an invalid entity"))
    return;
  }
  let { cp } = entity.Collision;
  let c = entity.Coordinates;
  let cpx = cp.X + c.X;
  let cpy = cp.Y + c.Y;
  return { X: cpx, Y: cpy };
