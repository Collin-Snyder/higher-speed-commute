...
/// MAKE AMBIENT MODULE ///
export function calculateSpeedConstant(entity) {
...
export function canHasFudge(x1, y1, x2, y2, fudge) {
    return abs(x2 - x1) <= fudge && abs(y2 - y1) <= fudge;
}
export function normalize(vectors) {
...
export function checkVelocityZero(v) {
    return v.X === 0 && v.Y === 0;
}
export function calculateSurroundingSquareCount(layers) {
    let total = 0;
    while (layers > 0) {
        total += layers * 8;
        layers--;
...
export function checkForMouseCollision(mx, my, ex, ey, ew, eh) {
    return mx >= ex && my >= ey && mx <= ex + ew && my <= ey + eh;
}
export function findCenteredElementSpread(p, e, n, style, c) {
...
export function randomNumBtwn(min, max) {
...
export function centerWithin(cx, cy, cw, ch, ew, eh) {
    let x = cw / 2 - ew / 2 + cx;
    let y = ch / 2 - eh / 2 + cy;
    return { x, y };
}
export function alignItems(cx, cy, cw, ch, entities, verticalAlignment, horizontalAlignment = "center", padding = 0) {
    let aligned = [];
...
function alignTop(cx, cy, cw, entities, padding, horizontalAlignment) {
    ...
    for (let entity of entities) {
        if (Array.isArray(entity)) {
            let maxH = max(...entity.map((e) => (isPositionable(e) ? e.Renderable.renderH : 0)));
            aligned.push(...justifyItems(cx, currentY, cw, maxH, entity, "spaceEvenly"));
            currentY += maxH + padding;
        }
        else {
            // if (entity.id === "resetButton") debugger;
...
export function justifyItems(cx, cy, cw, ch, entities, horizontalJustify, verticalJustify = "center", padding = 0) {
    let justified = [];
...
export function centerWithinOld(cx, cy, cw, ch, ew, eh, n, dir, style = "spaceEvenly") {
...
export function getCenterPoint(x, y, w, h) {
...
export function findDegFromVector(v) {
    const { X, Y } = v;
    if (X === 0 && Y === 0)
        return -1;
    if (X > 0 && Y < 0)
...
export function findRotatedVertex(vx, vy, cx, cy, d) {
...
export function getTileHitbox(x, y, w, h) {
    return [
        { X: x, Y: y },
        { X: x + w, Y: y },
        { X: x + w, Y: y + h },
...
export function checkCollision(hb1, hb2) {
    let next = 0;
...
export function checkSideCollision(hb, x1, y1, x2, y2) {
    let next = 0;
...
export function checkPointCollision(hb, px, py) {
    let collision = false;
    let next = 0;
    for (let curr = 0; curr < hb.length; curr++) {
        next = curr + 1;
